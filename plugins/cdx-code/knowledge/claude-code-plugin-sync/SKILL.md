...
# ClaudeCodePlugin Sync Scripts (Code)
...
Transforms codebase analysis (`.contextdx/boards/<board-slug>.json`) into ClaudeCodePlugin format and pushes to the API.

**Invocation:**
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/cdx-sync.js [options]
```
...
**Invocation:**
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/cdx-archetypes.js [options]
```
...
**Invocation:**
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/cdx-boards.js [options]
```
