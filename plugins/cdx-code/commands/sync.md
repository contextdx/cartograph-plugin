---
description: Sync architecture analysis to ClaudeCodePlugin
argument-hint: [--board <slug> | --all]
allowed-tools: Read, Bash(node:*)
---

Synchronize local architecture analysis to ClaudeCodePlugin, board by board.
...
Syncs a single board to ClaudeCodePlugin.
...
For each board to sync, run the ClaudeCodePlugin sync CLI with the board's analysis file and board slug:

```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/cdx-sync.js \
...
If syncing multiple boards, show a summary at the end.
