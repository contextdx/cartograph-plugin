...
- **Exit code 1** → stop and tell the user: "ClaudeCodePlugin not configured — run `/configure` first"
...
11. **Write to temp file** and save/push:
   ```bash
   node ${CLAUDE_PLUGIN_ROOT}/scripts/cdx-insights.js --save-insight /tmp/insight-<slug>.json --board-slug <primaryBoardSlug> --push
   ```
...
- **No config**: Report "ClaudeCodePlugin not configured — run /configure first"
